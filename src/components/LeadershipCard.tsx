...
const LeadershipCard: React.FC<LeadershipCardProps> = ({ image, name, title, company }) => {
  return (
    <div className="image-box group flex w-full max-w-sm md:max-w-md lg:max-w-lg items-center justify-center flex-col gap-6 rounded-2xl overflow-hidden shadow-md shadow-[#FB7602] transition-all duration-300 hover:bg-[#FB7602] text-textblue">
      <Image
        src={image}
        width={580}
        height={642}
      ...
      <div className="designation text-center flex flex-col gap-4 font-monte px-4">
        ...
        <div className='text-[16px] md:text-[18px] pb-8 md:pb-12 lg:pb-20 transition-colors duration-300'>
          <p className='text-[#FB7602] group-hover:text-[var(--textblue)]'>{title}</p>
